...
We begin with the Euler-Lagrange equations \eqref{theta1} and \eqref{theta2}. By grouping the angular acceleration terms ($\ddot{\theta}_1, \ddot{\theta}_2$) on the left-hand side and the remaining velocity and potential terms on the right, we obtain the following linear system:
...
There are many applicable methods we can utilize to solve for the angular accelerations $\ddot{\theta}$. For its ease, we will invert the mass matrix $M$. For a $2\times2$ matrix, the inverse is explicitly given by:
\[
...
Substituting the components of vector $b$ (from \eqref{eq:rearr1} and \eqref{eq:rearr2}) into these expressions provides the necessary system allowing for numerical solvers such as the RK4 method we will use.
...
Given the vector $X_n$ at time $t_n$ and a time step $h$, the next state $X_{n+1}$ is computed as a weighted average of four slope estimates:
\[
